    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form class="form-horizontal" method="POST" action="{{ route('trade_license_signboard_edit',['signboard'=>$signboard->id]) }}">
                    @csrf
                        ...
                        <div class="form-group row {{ $errors->has('sign_board_type') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" class="form-control" placeholder="সাইন বোর্ডের ধরন"
                                       name="sign_board_type" value="{{ empty(old('sign_board_type')) ? ($errors->has('sign_board_type') ? '' : $signboard->sign_board_type) : old('sign_board_type') }}">
                        ...
                        <div class="form-group row {{ $errors->has('sign_board_rate') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" class="form-control" placeholder="সাইন বোর্ড(ব.ফুট)"
                                       name="sign_board_rate" value="{{ empty(old('sign_board_rate')) ? ($errors->has('sign_board_rate') ? '' : $signboard->sign_board_rate ?? '') : old('sign_board_rate') }}">
