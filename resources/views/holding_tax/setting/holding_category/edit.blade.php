...
@section('title')
    হোল্ডিং মালিকানার ধরন পরিবর্তন
@endsection
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form class="form-horizontal" method="POST" action="{{ route('holding.holding_category.edit',['holdingCategory'=>$holdingCategory->id]) }}">
                    @csrf
                        ...
                        <div class="form-group row {{ $errors->has('name') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" class="form-control" placeholder="মালিকানার ধরন"
                                       name="name" value="{{ empty(old('name')) ? ($errors->has('name') ? '' : $holdingCategory->name) : old('name') }}">
                                @error('name')
                        ...
                        <div class="form-group row {{ $errors->has('taxable') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10" style="margin-top: 6px !important">
                                <div class="icheck-success d-inline">
                                    <input checked type="radio" id="active" name="taxable" value="1" {{ old('taxable',$holdingCategory->taxable) == '1' ? 'checked' : '' }}>
                                    <label for="active">
                                        হ্যা
                                    </label>
                                </div>
                                ...
                                <div class="icheck-danger d-inline">
                                    <input type="radio" id="inactive" name="taxable" value="0" {{ old('taxable',$holdingCategory->taxable) == '0' ? 'checked' : '' }}>
                                    <label for="inactive">
                                        না
                                    </label>
                                </div>
                        ...
                        <div class="form-group row {{ $errors->has('tax_rate') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" class="form-control" placeholder="করের হার(%)"
                                       name="tax_rate" value="{{ empty(old('tax_rate')) ? ($errors->has('tax_rate') ? '' : $holdingCategory->tax_rate ?? '') : old('tax_rate') }}">
                                @error('tax_rate')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
