@extends('layouts.app')
@section('title','আয়/ব্যয় সংযুক্তি')
@section('style')
    <style>
        ...
        <div class="col-md-12">
            ...
            <div class="card card-outline card-default">
                <div class="card-header">
                    <h3 class="card-title">আয়/ব্যয় সংযুক্তির তথ্য</h3>
                </div>
                <!-- /.card-header -->
                <!-- form start -->
                <form enctype="multipart/form-data" action="{{ route('income_expenditure.create') }}" class="form-horizontal"
                      method="post">
                    @csrf
                    <div class="card-body">
                        ...
                        <div class="form-group row {{ $errors->has('voucher_no') ? 'has-error' :'' }}">
                            <label for="voucher_no" class="col-sm-3 col-form-label">ভাউচার/ চালান নম্বর</label>
                            <div class="col-sm-9">
                        ...
                        <div class="form-group row {{ $errors->has('cheque_amount') ? 'has-error' :'' }}">
                            <label for="cheque_amount" class="col-sm-3 col-form-label">চেকের টাকা</label>
                            <div class="col-sm-9">
                                <input type="text" value="{{ old('cheque_amount',0) }}" name="cheque_amount" class="form-control"
                        ...
                        <div class="form-group row {{ $errors->has('contractor') ? 'has-error' :'' }}">
                            <label for="contractor" class="col-sm-3 col-form-label">ঠিকাদার</label>
                            <div class="col-sm-9">
                                <select name="contractor" class="form-control select2" id="contractor">
                                    <option value="">ঠিকাদার নির্ধারণ</option>
                                    @foreach($employees as $employee)
                                        <option {{ old('contractor') == $employee->eid ? 'selected' : '' }} value="{{ $employee->eid }}">{{ $employee->name }}</option>
                                    @endforeach
                                </select>
                                @error('contractor')
                                <span class="help-block">{{ $message }}</span>
                ...
                if (financialYear != '') {
                    $.ajax({
                        ...
                        if(response.success == false){
                            if(response.budget_type == 2){
                                if(response.amount > 0){
                                    $('#submit-btn').removeAttr("disabled");
                                    $('#budget_message').text('বাজেটের পরিমানের চেয়ে খরচের পরিমান বেশি দেওয়া যাবে না। অবশিষ্ট বাজেটঃ '+ jsNumberFormat(response.amount));
                                    replaceNumbersWithBengali('.budget_message');
                                }else{
                                    $('#budget_message').text('এই অর্থবছরের জন্যে এই খাতে বাজেট অ্যাড করুন');
                                    $('#submit-btn').attr("disabled","disabled");
                                }
                            }else{
                                $('#budget_message').text(' ');
            ...
            $("#sub_sector_type").change(function () {
                ...
                if (subSectorTypeId != '') {
                    $.ajax({
                        ...
                        data: {subSectorTypeId: subSectorTypeId,sectorTypeId:sectorTypeId,upangshoId:upangshoId}
                    }).done(function (data) {
                        $.each(data, function (index, item) {
