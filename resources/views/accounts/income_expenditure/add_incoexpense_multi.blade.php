...
@section('content')
            ...
            <div class="btn-group btn-scroll-area" role="group" style="width: 100%;overflow: auto">
                @foreach ($upangshos as $upangsho)
                    <a style="white-space: nowrap"
                        href="{{ route('multi_income_expense_add', ['id' => $upangsho->upangsho_id, 'inOut' => 1]) }}"
                        class="btn  {{ $upangshoId == $upangsho->upangsho_id && $inOut == 1 ? 'btn-success' : 'btn-outline-success' }}">{{ $upangsho->upangsho_name }}
                        - আয়</a>
                    <a style="white-space: nowrap"
                        href="{{ route('multi_income_expense_add', ['id' => $upangsho->upangsho_id, 'inOut' => 2]) }}"
                        class="btn  {{ $upangshoId == $upangsho->upangsho_id && $inOut == 2 ? 'btn-success' : 'btn-outline-success' }}">{{ $upangsho->upangsho_name }}
                        - বায়</a>
                @endforeach
            </div>
        </div>
            ...
            <section class="card card-default">
                ...
                <div class="card-body">
                    <form role="form" class="cmxform form-horizontal tasi-form" id="MenuForm" method="post"
                        id="commentForm"
                        action="{{ route('multi_income_expense_add', ['id' => $upangshoId, 'inOut' => $inOut]) }}">
                        @csrf
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label for="economicalyear" class="control-label"><strong>অর্থ বছর <span
                                                class="text-danger">*</span></strong></label>
                                    <select style="color: red !important;" class="form-control select2" id="economicalyear"
                                        name="year" required>
                                        <option value="">অর্থ বছর নির্ধারণ</option>
                                        @for ($i = 2023; $i <= date('Y'); $i++)
                                            <option
                                                {{ date('Y') . '-' . substr(date('Y') + 1, -2) == $i . '-' . substr($i + 1, -2) ? 'selected' : '' }}
                                                value="{{ $i }}-{{ substr($i + 1, -2) }}">
                                                {{ enNumberToBn($i) }}-{{ enNumberToBn($i + 1) }}</option>
                                        @endfor
                                    </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" id="bank_id" name="bank_id" required>
                                        ...
                                        @foreach ($bank as $data)
                                            <option
                                                @if ($lastPostingData) {{ $lastPostingData->bank_id == $data->bank_id ? 'selected' : '' }} @endif
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" autocomplete="off"
                                        value="{{ $lastPostingData ? \Carbon\Carbon::parse($lastPostingData->receive_datwe)->format('d-m-Y') : '' }}"
                                        class="form-control date-picker" placeholder="প্রাপ্তি / প্রদান তারিখ"
                                        id="receive_date" name="receive_date" required>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label for="vourcher_no" class="control-label">
                                        <strong>রশিদ নং/বিল নং<span class="text-danger">*</span></strong></label>
                                    <input type="text" class="form-control" placeholder="রশিদ নম্বর" id="vourcher_no"
                                        name="vourcher_no">
                            ...
                            <div class="col-md-4 item-hide">
                                <div class="form-group">
                                    ...
                                    <input type="text" value="{{ $lastPostingData ? $lastPostingData->check_no : '' }}"
                                        class="form-control" placeholder="চেক নম্বর" id="check_no" name="check_no"
                                        minlength="2">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" placeholder="মন্তব্য" id="note"
                                        name="note" minlength="2">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="receiver_name" class="form-control"
                                        placeholder="প্রাপকের নাম" name="receiver_name">
                                </div>
                            </div>
                                    ...
                                    <table class="table table-bordered">
                                        <thead>
                                            <tr class="item-hide-s">
                                                <td colspan="2">
                                                    <div class="form-group">
                                                        <input type="text" class="form-control" placeholder="মোট টাকা"
                                                            id="total_amount_define">
                                            ...
                                            <tr>
                                                <th width="40%" class="text-center">খাত <span
                                        ...
                                        <tbody class="app-container" id="app-container">
                                            <tr class="app-item">
                                                <td>
                                                    ...
                                                    <input type="hidden" class="khat_type_type_id"
                                                        name="khtattypetype_id[]">
                                                    <div class="form-group">
                                                        <select class="form-control select2 khat_id" name="khat_id[]"
                                                            required>
                                                            <option value="">খাত সার্চ করুন</option>
                                                            @foreach ($khats as $khat)
                                                                <option value="{{ $khat->khat_id }}">{{ $khat->serilas }}
                                                                    {{ $khat->khat_name }}</option>
                                                            @endforeach
                                                        </select>
                                                ...
                                                <td>
                                                    <div class="form-group">
                                                        <input type="text" class="form-control text-center"
                                                            placeholder="খাতের বিবরণ" name="khat_des[]">
                                                    </div>
                                                </td>
                                                <td>
                                                    <div class="form-group">
                                                        <input type="text"
                                                            class="form-control text-center single_amount"
                                                            placeholder="টাকা" name="amount[]" required>
                                                    </div>

                                                </td>
                                                <td class="item-hide">
                                                    <div class="form-group">
                                                        <input type="text" step="any" class="form-control jamanot"
                                                            placeholder="জামানত" name="jamanot[]">
                                                        <input type="text" step="any" class="form-control vat"
                                                            placeholder="ভ্যাট" name="vat[]">
                                                        <input type="text" class="form-control tax"
                                                            placeholder="আয় কর" name="tax[]">
                                                    </div>
                                                </td>

                                                <td class="item-hide">
                                                    <div class="form-group item_add item_add_check">
                                                        <input type="text" step="any"
                                                            class="form-control jamanot_check" placeholder="চেক নম্বর"
                                                            name="jamanot_check_no[]">
                                                        <input type="text" step="any" class="form-control"
                                                            placeholder="চেক নম্বর" name="vat_check_no[]">
                                                        <input type="text" class="form-control"
                                                            placeholder="চেক নম্বর" name="tax_check_no[]">
                                                    </div>
                                                </td>
                                                <td style="vertical-align: middle" class="text-center">
                                                    <a role="button" class="btn btn-danger btn-sm btn-remove"><i
                                                            class="fa fa-trash"></i>
                                                    </a>
                                                </td>
                                            </tr>
                                        ...
                                        <tfoot>
                                            <tr>
                                                ...
                                                <td class="text-right" colspan="">
                                                    <div class="form-group">
                                                        <input type="text" class="form-control text-center"
                                                            id="ribet" placeholder="রিবেট">
                                                    </div>
                                                </td>
                                                <td class="text-right" colspan="">
                                                    <div class="form-group">
                                                        <input type="text" class="form-control text-center"
                                                            id="sarcharge" placeholder="সারচার্জ">
                                                    </div>
                                                </td>
                                            ...
                                            <tr>
                                                <th colspan="5">
                                                    <button type="submit" id="smbtn"
                                                        class="btn btn-success bg-gradient-success">Submit</button>
    ...
    <template id="app-template">
        <tr class="app-item">
            ...
            <td>
                <div class="form-group">
                    <input type="text" class="form-control single_amount text-center" placeholder="টাকা"
            ...
            <td class="item-hide">
                <div class="form-group">
                    <input type="text" step="any" class="form-control text-center jamanot" placeholder="জামানত"
                        name="jamanot[]">
                    <input type="text" step="any" class="form-control text-center vat" placeholder="ভ্যাট"
                        name="vat[]">
                    <input type="text" class="form-control text-center tax" placeholder="আয় কর" name="tax[]">
                </div>
            </td>
            <td style="vertical-align: middle" class="text-center"> <a role="button"
                    class="btn btn-danger btn-sm btn-remove"><i class="fa fa-trash"></i></a></td>
    ...
    <script>
        $(function() {
            ...
            $('body').on('change', '.khat_type_id', function() {
                ...
                itemKhatType.closest('.app-item').find('.khat_type_type_id').html(
                    '<option value="">উপখাত টাইপ নির্ধারণ</option>');
                itemKhatType.closest('.app-item').find('.khat_id').html(
                ...
                if (sectorId != '') {
                    $.ajax({
                        ...
                        data: {
                            ...
                            upangshoId: upangshoId
                        }
                    }).done(function(data) {
                        $.each(data, function(index, item) {
                            itemKhatType.closest('.app-item').find('.khat_type_type_id')
                                .append('<option value="' + item.tax_id + '">' + item
                                    .tax_name2 + '</option>');
                        });
                    });
                }
            ...
            $('body').on('change', '.khat_type_type_id', function() {
                ...
                itemKhatTypeType.closest('.app-item').find('.khat_id').html(
                    '<option value="">খাত নির্ধারণ</option>');
                if (subSectorTypeId != '') {
                    $.ajax({
                        method: "GET",
                        ...
                        data: {
                            ...
                            subSectorTypeId: subSectorTypeId
                        }
                    }).done(function(data) {
                        $.each(data, function(index, item) {
                            itemKhatTypeType.closest('.app-item').find('.khat_id').append(
                                '<option value="' + item.khat_id + '">' + (item
                                    .serilas != null ? item.serilas : '') + '' + item
                                .khat_name + '</option>');
                        });
                    });
                }
            ...
            $('body').on('change', '.khat_id', function() {
                ...
                if (khatId != '') {
                    $.ajax({
                        ...
                        itemKhat.closest('.app-item').find('.khat_type_type_id').val(data
                            .tax_type_type_id);
                    });
                }
            })
            ...
            $('body').on('keyup', '.single_amount,.jamanot,.vat,.tax,#sarcharge,#ribet,#total_amount_define',
                function() {
                    calculate();
                });
                        ...
                        success: function(data) {
                            $.each(data, function(index, item) {
                                if (OldBranchId == item.branch_id)
                                    $('#branch_id').append('<option value="' + item
                                        .branch_id + '" selected>' + item.branch_name +
                                        '</option>');
                                else
                                    $('#branch_id').append('<option value="' + item
                        ...
                        success: function(data) {
                            $.each(data, function(index, item) {
                                if (OldBankAccountId == item.bank_details_id)
                                    $('#acc_no').append('<option value="' + item
                                        .bank_details_id + '" selected>' + item.acc_no +
                                        '</option>');
                                else
                                    $('#acc_no').append('<option value="' + item
                                        .bank_details_id + '">' + item.acc_no +
                                        '</option>');
                            });
                        }
                    });
        ...
        function calculate() {
            ...
            $('#total-amount').html(parseFloat(subTotal) + parseFloat(jamanotVatTaxTotal) + parseFloat(sarcharge) -
                parseFloat(ribet).toFixed(2));
            ...
            $("#total_amount_define").val(parseFloat(hiddenDefAmount) - (parseFloat(subTotal) + parseFloat(
                jamanotVatTaxTotal) + parseFloat(sarcharge) - parseFloat(ribet)));
