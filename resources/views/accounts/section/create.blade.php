@extends('layouts.app')
@section('title','উপাংশ যুক্ত করুন')
@section('content')
    <div class="row">
        <!-- left column -->
        <div class="col-md-12">
            ...
            <div class="card card-outline card-default">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form enctype="multipart/form-data" action="{{ route('section.create') }}" class="form-horizontal" method="post">
                    @csrf
                    <div class="card-body">
                        <div class="form-group row {{ $errors->has('name') ? 'has-error' :'' }}">
                            <label for="name" class="col-sm-2 col-form-label">উপাংশ নাম <span class="text-danger">*</span></label>
                            <div class="col-sm-10">
                                <input type="text" value="{{ old('name') }}" name="name" class="form-control" id="name" placeholder="উপাংশ নাম">
                                @error('name')
                        ...
                        <div class="form-group row {{ $errors->has('code') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" value="{{ old('code') }}" name="code" class="form-control" id="code" placeholder="উপাংশ কোড">
                                @error('code')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
