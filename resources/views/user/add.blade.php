@extends('layouts.app')
@section('title','ব্যবহারকারী যুক্ত করুন')
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-md-12">
            ...
            <div class="card card-outline card-default">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form enctype="multipart/form-data" action="{{ route('user.add') }}" class="form-horizontal" method="post">
                    @csrf
                    <div class="card-body">
                        <div class="form-group row {{ $errors->has('role') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <select name="role" id="role" class="form-control select2">
                                    <option value="">মডিউল নিরধারন</option>
                                    <option {{ old('role') == 1 ? 'selected' : '' }} value="1">আডমিন</option>
                                    @foreach($sisterConcerns as $sisterConcern)
                                    <option {{ old('role') == $sisterConcern->role ? 'selected' : '' }} value="{{ $sisterConcern->role }}">{{ $sisterConcern->name }}</option>
                                    @endforeach
                                </select>
                                @error('role')
                        ...
                        <div class="form-group row {{ $errors->has('sub_role') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <select name="sub_role" id="sub_role" class="form-control select2">
                                    ...
                                    <option {{ old('sub_role') == 1 ? 'selected' : '' }} value="1">আডমিন</option>
                                    @if(auth()->user()->role == \App\Enumeration\Role::$COLLECTION)
                                    <option {{ old('sub_role') == 2 ? 'selected' : '' }} value="2">আদায়কারী</option>
                                    <option {{ old('sub_role') == 3 ? 'selected' : '' }} value="3">ক্যাশিয়ার</option>
                                    @endif
                                </select>
                                @error('sub_role')
                                <span class="help-block">{{ $message }}</span>
                        ...
                        <div class="form-group row {{ $errors->has('name') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" value="{{ old('name') }}" name="name" class="form-control" id="name" placeholder="নাম লিখন">
                                @error('name')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="form-group row {{ $errors->has('username') ? 'has-error' :'' }}">
                            ...
                            <div class="col-sm-10">
                                <input type="text" value="{{ old('username') }}" name="username" class="form-control" id="username" placeholder="লগিন ইউজার নাম লিখন">
                                @error('username')
                                <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="form-group row {{ $errors->has('password') ? 'has-error' :'' }}">
                            <label for="password" class="col-sm-2 col-form-label">পাসওয়াড <span class="text-danger">*</span></label>
                            <div class="col-sm-10">
                        ...
                        <div class="form-group row {{ $errors->has('password_confirmation') ? 'has-error' :'' }}">
                            <label for="password_confirmation" class="col-sm-2 col-form-label">পাসওয়াড নিশ্চিতকরণ <span class="text-danger">*</span></label>
                            <div class="col-sm-10">
