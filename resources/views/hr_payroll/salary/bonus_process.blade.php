@extends('layouts.app')
@section('title','বোনাস প্রস্তুত করুন')
@section('content')
    <div class="row">
        <!-- left column -->
        <div class="col-md-12">
            ...
            <div class="card card-outline card-default">
                <div class="card-header">
                    <h3 class="card-title">বোনাস প্রস্তুতের তথ্য</h3>
                </div>
                <!-- /.card-header -->
                <!-- form start -->
                <form action="{{ route('employee_bonus_process') }}">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-3">
                            ...
                            <div class="col-md-3">
                                <div class="form-group {{ $errors->has('bonus_type') ? 'has-error' :'' }}">
                                    <label for="bonus_type">বোনাস <span
                                            class="text-danger">*</span></label>
                                        <select required class="form-control select2" name="bonus_type" id="bonus_type">
                                            <option value="">বোনাস নির্ধারণ</option>
                                            @foreach($bonusTypes as $bonusType)
                                                <option
                                                    value="{{ $bonusType->id }}" {{ old('bonus_type') == $bonusType->id ? 'selected' : '' }}>{{ $bonusType->name }}</option>
                                            @endforeach
                                        </select>
                                </div>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="process_date">প্রস্তুতের তারিখ <span class="text-danger">*</span></label>
                                    <input required type="text" id="process_date" autocomplete="off"
                                           name="process_date" class="form-control date-picker"
                                           placeholder="প্রস্তুতের তারিখ লিখুন" value="{{ request()->get('process_date')  }}">
                                </div>
                            </div>
                            <div class="col-md-2">
                                <div class="form-group">
                                    ...
                                    <input type="submit" name="search"
                                           class="btn btn-success bg-gradient-success form-control" value="প্রস্তুত করুন">
                                </div>
                            </div>
                        </div>
                    </div>
